...
plot_cov <- function(sobj, type = "relpos", ordering = TRUE, facetting = TRUE) {
  ...
  hw_idx <- table(gsub("\\d*$", "", levels(dta$v1)))
  h_idx <- hw_idx[grepl("X|Z", names(hw_idx))]
  v_idx <- hw_idx[grepl("Y|W", names(hw_idx))]
  ...
  if (facetting) {
    plt <- plt +
      geom_hline(yintercept = h_idx + 0.5, size = 3, color = "whitesmoke") +
      geom_vline(xintercept = v_idx + 0.5, size = 3, color = "whitesmoke")
  }
  return(plt)
}
